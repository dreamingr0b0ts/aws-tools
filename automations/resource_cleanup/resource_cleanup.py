...
def find_old_snapshots(ec2, days):
    ...
    return [(s["SnapshotId"], f'{s["StartTime"].date()} {s.get("VolumeSize", "?")}GiB')
            for s in snaps if s["StartTime"] < cutoff]
...
def find_idle_load_balancers(region):
    ...
    for lb in elbv2.describe_load_balancers()["LoadBalancers"]:
        ...
        registered = sum(
            len(elbv2.describe_target_health(TargetGroupArn=tg["TargetGroupArn"])
                ["TargetHealthDescriptions"])
            for tg in elbv2.describe_target_groups(LoadBalancerArn=arn)["TargetGroups"])
        if registered == 0:
...
def scan_region(region, days, delete):
    ...
    actions = [
        ("unattached volume", find_unattached_volumes(ec2),
         lambda ref: ec2.delete_volume(VolumeId=ref), "deleted"),
        ("old snapshot", find_old_snapshots(ec2, days),
         lambda ref: ec2.delete_snapshot(SnapshotId=ref), "deleted"),
        ("unused EIP", find_unused_eips(ec2),
         lambda ref: ec2.release_address(AllocationId=ref), "released"),
    ]
...
def main():
    ...
    p.add_argument("--days", type=int, default=90,
                   help="Snapshot age threshold in days (default 90).")
